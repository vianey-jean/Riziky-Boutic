  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        const promotionalProducts = response.data.filter(product => 
          product.promotion && 
        ...
        // Calculer le prix maximum parmi les produits en promotion
        const highestPrice = Math.max(...promotionalProducts.map(p => p.price), 200);
        setMaxPrice(Math.ceil(highestPrice / 10) * 10);
        setPriceRange([0, highestPrice]);
        
        setFilteredProducts(promotionalProducts);
        setSortedProducts(promotionalProducts);
      } catch (error) {
      ...
      <div className="min-h-screen bg-gradient-to-br from-neutral-50 via-white to-neutral-50 dark:from-neutral-950 dark:via-neutral-900 dark:to-neutral-950">
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
            <div>
              <h1 className="text-3xl font-bold mb-2 text-neutral-800 dark:text-neutral-100">Tous nos produits en promotion</h1>
              <p className="text-muted-foreground">
                {sortedProducts.length} produit{sortedProducts.length > 1 ? 's' : ''} en promotion trouvé{sortedProducts.length > 1 ? 's' : ''}
              </p>
            </div>
