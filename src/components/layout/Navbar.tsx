...
const Navbar = () => {
  ...
  return <nav aria-label="Navigation principale" className="border-b py-4 bg-slate-300">
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between">
        <Link to="/" className="flex items-center" aria-label="Page d'accueil">
            <img src={logo} alt="Riziky Boutique" className="h-20 w-auto text-red-600 text-3xl font-bold" />
          </Link>
          ...
          <div className="hidden md:flex items-center space-x-4 flex-1 max-w-md mx-8">
            <div className="relative w-full" ref={searchRef} role="search">
              ...
              <Input id="search-desktop" type="search" placeholder="Rechercher des produits..." value={searchTerm} onChange={e => {
            ...
            }} aria-label="Rechercher des produits" className="w-full pl-10 rounded-2xl bg-orange-50" />
              {isSearching ? <div className="absolute right-3 top-2.5 h-4 w-4 animate-spin rounded-full border-2 border-gray-300 border-t-brand-blue"></div> : <Search className="absolute left-3 top-2.5 h-4 w-4 text-muted-foreground" aria-hidden="true" />}
              {renderSearchResults()}
            </div>
          </div>
          ...
          <div className="hidden md:flex items-center space-x-4">
            <Link to="/favoris" className="relative">
              <Button variant="ghost" size="icon" className="nav-icon rounded-full bg-teal-300 hover:bg-teal-200">
                <Heart className="h-7 w-7" />
              </Button>
              {favoriteCount > 0 && <Badge variant="destructive" className="absolute -top-2 -right-2 w-5 h-5 flex items-center justify-center p-0 text-xs rounded-full">
                  {favoriteCount}
                </Badge>}
            </Link>

            <Link to="/panier" className="relative">
              <Button variant="ghost" size="icon" className="nav-icon rounded-full bg-violet-400 hover:bg-violet-300">
            ...
            {isAuthenticated ? <DropdownMenu>
                ...
                <Button variant="ghost" size="icon" className="nav-icon rounded-full text-inherit bg-rose-600 hover:bg-rose-500">
                  <User className="h-7 w-7" />
                </Button>
              </Link>}
          </div>
              ...
              <SheetContent side="right" className="w-[300px] sm:w-[400px]">
                <nav className="flex flex-col h-full">
                  <div className="flex-1 py-4">
                    <div className="mb-6">
                      <Input type="text" placeholder="Rechercher des produits..." className="w-full pl-10" value={searchTerm} onChange={handleSearchChange} />
                      ...
                      <div>
                        <SheetClose asChild>
                          <Link to="/favoris" className="flex items-center hover:text-primary">
                            <Heart className="mr-2 h-6 w-6 nav-icon" />
                            <span>Mes favoris</span>
                            {favoriteCount > 0 && <Badge variant="outline" className="ml-2 text-red-600">
                                {favoriteCount}
                              </Badge>}
                          </Link>
                        </SheetClose>
                      </div>
                      {isAuthenticated && <div className="pb-4 border-b">
                          ...
                          <ul className="space-y-3">
                            ...
                            <li>
                              <button className="flex items-center text-sm text-red-600 hover:text-red-800" onClick={() => {
                            logout();
                            setIsOpen(false);
                          }}>
        ...
        <div className="md:hidden mt-4 relative" ref={searchRef}>
          <Input type="text" placeholder="Rechercher des produits..." className="w-full pl-10" value={searchTerm} onChange={handleSearchChange} />
          {isSearching ? <div className="absolute right-3 top-2.5 h-4 w-4 animate-spin rounded-full border-2 border-gray-300 border-t-brand-blue"></div> : <Search className="absolute left-3 top-2.5 h-4 w-4 text-muted-foreground" />}
          {renderSearchResults()}
        </div>
        ...
        <div className="hidden md:flex mt-4 space-x-4 overflow-x-auto py-2 justify-center" role="navigation" aria-label="Catégories">
          <ul className="flex space-x-4">
            {categories.map(cat => <li key={cat}>
                <Link to={`/categorie/${cat}`} className="text-sm whitespace-nowrap text-red-800 hover:text-red-600 capitalize">
            ...
            <AccordionItem value="categories">
              ...
              <AccordionContent>
                <div className="grid grid-cols-2 gap-2 pt-2">
                  {categories.map(cat => <Link key={cat} to={`/categorie/${cat}`} className="text-sm py-1 px-2 rounded-md bg-gray-50 hover:bg-gray-100 capitalize">
                      {cat.charAt(0).toUpperCase() + cat.slice(1)}
                    </Link>)}
                </div>
              </AccordionContent>
